                            ...
                            <div class="col-md-6">
                                <form method="GET" action="{{ route('admin.photos.user-submissions') }}" class="form-inline">
                                    ...
                                    <div class="input-group">
                                        <input type="text" name="s" class="form-control form-control-sm" placeholder="Search photo or user..." value="{{ request('s') }}">
                        ...
                        <div class="table-responsive">
                            <table class="table table-bordered table-striped">
                                ...
                                <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td class="text-center">
                                                @if($photo->drive_file_id)
                                                    <img src="https://drive.google.com/thumbnail?id={{ $photo->drive_file_id }}&sz=w100" style="width: 60px; height: 45px; object-fit: cover; border-radius: 4px; cursor: pointer;" onclick="previewPhoto('{{ $photo->image_url }}', '{{ $photo->drive_file_id }}', '{{ addslashes($photo->title) }}')" alt="thumb">
                                                @elseif($photo->image_path)
                                                    <img src="{{ $photo->image_url }}" style="width: 60px; height: 45px; object-fit: cover; border-radius: 4px; cursor: pointer;" onclick="previewPhoto('{{ $photo->image_url }}', null, '{{ addslashes($photo->title) }}')" alt="thumb">
                                                @else
                                                    <i class="fa fa-image text-muted" style="font-size: 24px;"></i>
                                                @endif
                                            </td>
                                            <td>
                                                <strong style="color: #fff;">{{ $photo->title }}</strong>
                                            ...
                                            <td class="text-center" style="white-space: nowrap;">
                                                ...
                                                @if($photo->status !== 'active')
                                                    <a href="{{ route('admin.photos.user-submissions.approve', $photo->id) }}" class="btn btn-sm btn-success m-r-5" data-toggle="tooltip" title="Approve & Publish Live" onclick="return confirm('Approve this photo and publish to library?')">
                                                        <i class="fa fa-check"></i> Approve
                                                    </a>
                                                @endif

                                                @if($photo->status !== 'rejected')
                                                    <a href="{{ route('admin.photos.user-submissions.reject', $photo->id) }}" class="btn btn-sm btn-warning m-r-5" data-toggle="tooltip" title="Reject Submission" onclick="return confirm('Reject this photo submission?')">
                                    ...
                                    @empty
                                        <tr>
                                            <td colspan="9" class="text-center py-4">No user-submitted photos found.</td>
                                        </tr>
                                    @endforelse
                                </tbody>
                            </table>
    ...
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content" style="background: #1a2234; border: 1px solid #32383e; color: #fff; border-radius: 8px;">
            ...
            <div class="modal-body p-3 text-center" style="background: #0d1117;">
                <img id="photoModalImg" src="" style="max-width: 100%; max-height: 500px; object-fit: contain; border-radius: 6px;" alt="preview">
            </div>
        </div>
    </div>
</div>
