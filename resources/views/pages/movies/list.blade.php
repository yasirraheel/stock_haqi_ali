                    ...
                    <div class="col-lg-12">
                        ...
                        <div class="custom_select_filter">
                            <select class="selectpicker show-menu-arrow" id="filter_by_lang">
                                ...
                                @foreach (\App\Language::where('status', '1')->orderBy('id')->get() as $lang_data)
                                    <option value="{{ URL::to('movies/?lang_id=' . $lang_data->id) }}"
                                        @if (isset($_GET['lang_id']) && $_GET['lang_id'] == $lang_data->id) selected @endif>
                                        {{ stripslashes($lang_data->language_name) }}</option>
                                @endforeach
                        ...
                        <div class="custom_select_filter">
                            <select class="selectpicker show-menu-arrow" id="filter_by_genre">
                                ...
                                @foreach (\App\Genres::where('status', '1')->orderBy('id')->get() as $genres_data)
                                    <option value="{{ URL::to('movies/?genre_id=' . $genres_data->id) }}"
                                        @if (isset($_GET['genre_id']) && $_GET['genre_id'] == $genres_data->id) selected @endif>
                                        {{ stripslashes($genres_data->genre_name) }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="custom_select_filter">
                            <select class="selectpicker show-menu-arrow" id="filter_list" required>
                                <option value="?filter=new" @if (isset($_GET['filter']) && $_GET['filter'] == 'new') selected @endif>
                                    {{ trans('words.newest') }}</option>
                                <option value="?filter=old" @if (isset($_GET['filter']) && $_GET['filter'] == 'old') selected @endif>
                                    {{ trans('words.oldest') }}</option>
                                <option value="?filter=alpha" @if (isset($_GET['filter']) && $_GET['filter'] == 'alpha') selected @endif>
                                    {{ trans('words.a_to_z') }}</option>
                                <option value="?filter=rand" @if (isset($_GET['filter']) && $_GET['filter'] == 'rand') selected @endif>
                                    {{ trans('words.random') }}</option>
                            </select>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="col-lg-2 col-md-3 col-sm-4 col-xs-12 col-6">
                        <div class="single-video">
                                ...
                                @else
                                    @if ($movies_data->video_access == 'Paid')
                                        <a href="{{ URL::to('movies/details/' . $movies_data->video_slug . '/' . $movies_data->id) }}"
                                            ...
                                            data-target="#loginAlertModal" title="{{ $movies_data->video_title }}">
                                        @else
                                            <a href="{{ URL::to('movies/details/' . $movies_data->video_slug . '/' . $movies_data->id) }}"
                                                title="{{ $movies_data->video_title }}">
                                    @endif
                            ...
                            <div class="video-img">
                                ...
                                <span
                                    class="video-item-content">{{ Str::limit(stripslashes($movies_data->video_title), 20) }}</span>
