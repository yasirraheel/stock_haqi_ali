<?php $__env->startSection('head_title', stripslashes($audio->title).' | '.getcong('site_name') ); ?>

<?php if($audio->description): ?>
  <?php $__env->startSection('head_description', Str::limit(stripslashes($audio->description),160)); ?>
<?php endif; ?>

<?php $__env->startSection('head_image', URL::to('/site_assets/images/audio-placeholder.jpg') ); ?>
    ...
    <div class="row">
        ...
        <div class="detail-poster-area">
          ...
          <div class="dtl-poster-img" style="background: linear-gradient(90deg, #ff8508, #fd0575); height: 250px; display: flex; align-items: center; justify-content: center; color: white; font-size: 64px; border-radius: 8px; position: relative; margin-bottom: 20px;">
            <?php if($audio->license_price && $audio->license_price > 0): ?>
              <div class="premium-badge" style="position: absolute; top: 15px; right: 15px; background: linear-gradient(45deg, #ffd700, #ffed4e); color: #333; padding: 8px 12px; border-radius: 20px; font-size: 11px; font-weight: bold; z-index: 10; box-shadow: 0 2px 8px rgba(255,215,0,0.3);">
                <i class="fa fa-crown"></i> Premium - <?php echo e(getcong('currency_symbol')); ?><?php echo e(number_format($audio->license_price, 2)); ?>
          ...
          <div class="audio-metadata" style="display: flex; flex-wrap: wrap; gap: 15px; align-items: center; margin-bottom: 20px; padding: 15px; background: #1a1a2e; border-radius: 8px; border: 1px solid #2d2d44;">
            <div class="metadata-stats" style="display: flex; gap: 15px; align-items: center;">
              <span style="color: #b5b5b5; font-size: 12px;"><i class="fa fa-eye"></i> <?php echo e(number_format_short($audio->views_count)); ?> Views</span>
              <?php if($audio->created_at): ?>
                <span style="color: #b5b5b5; font-size: 12px;"><i class="fa fa-calendar-alt"></i> <?php echo e(date('M d Y', strtotime($audio->created_at))); ?></span>
              <?php endif; ?>
              <?php if($audio->duration): ?>
                <span style="color: #b5b5b5; font-size: 12px;"><i class="fa fa-clock"></i> <?php echo e($audio->duration); ?></span>
              <?php endif; ?>
            </div>

            <div class="metadata-actions" style="display: flex; gap: 10px; margin-left: auto;">
              <?php if(Auth::check()): ?>
                <?php if(check_watchlist(Auth::user()->id,$audio->id,'Audio')): ?>
                  <a href="<?php echo e(URL::to('watchlist/remove')); ?>?post_id=<?php echo e($audio->id); ?>&post_type=Audio" class="btn btn-sm" style="background: #fe0278; color: white; padding: 5px 12px; border-radius: 15px; font-size: 11px; text-decoration: none;"><i class="fa fa-check"></i> Remove from Watchlist</a>
                <?php else: ?>
                  <a href="<?php echo e(URL::to('watchlist/add')); ?>?post_id=<?php echo e($audio->id); ?>&post_type=Audio" class="btn btn-sm" style="background: #2dbe60; color: white; padding: 5px 12px; border-radius: 15px; font-size: 11px; text-decoration: none;"><i class="fa fa-plus"></i> Add to Watchlist</a>
                <?php endif; ?>
              <?php else: ?>
                <a href="<?php echo e(URL::to('watchlist/add')); ?>?post_id=<?php echo e($audio->id); ?>&post_type=Audio" class="btn btn-sm" style="background: #2dbe60; color: white; padding: 5px 12px; border-radius: 15px; font-size: 11px; text-decoration: none;"><i class="fa fa-plus"></i> Add to Watchlist</a>
              <?php endif; ?>
              ...
              <div class="modal-content bg-dark-2 text-light">
                ...
                <div class="modal-body p-4">
                  <div class="social-media-modal">
                    <ul>
                      <li><a title="Sharing" href="https://www.facebook.com/sharer/sharer.php?u=<?php echo e(URL::to('audio/details/'.$audio->id)); ?>" class="facebook-icon" target="_blank"><i class="ion-social-facebook"></i></a></li>
                      <li><a title="Sharing" href="https://twitter.com/intent/tweet?text=<?php echo e($audio->title); ?>&amp;url=<?php echo e(URL::to('audio/details/'.$audio->id)); ?>" class="twitter-icon" target="_blank"><i class="ion-social-twitter"></i></a></li>
                      <li><a title="Sharing" href="https://www.instagram.com/?url=<?php echo e(URL::to('audio/details/'.$audio->id)); ?>" class="instagram-icon" target="_blank"><i class="ion-social-instagram"></i></a></li>
                      <li><a title="Sharing" href="https://wa.me?text=<?php echo e(URL::to('audio/details/'.$audio->id)); ?>" class="whatsapp-icon" target="_blank"><i class="ion-social-whatsapp"></i></a></li>
                    </ul>
                  </div>
                </div>
              </div>
      ...
      <div class="col-lg-5 col-md-12 col-sm-12 col-xs-12 mb-4">
        <div class="poster-dtl-item">
          <h2><?php echo e(stripslashes($audio->title)); ?></h2>
          <ul class="dtl-list-link">
            <?php if($audio->genre): ?>
              <li><a href="<?php echo e(URL::to('audio/?genre='.urlencode($audio->genre))); ?>" title="<?php echo e($audio->genre); ?>"><?php echo e($audio->genre); ?></a></li>
            <?php endif; ?>
            <?php if($audio->format): ?>
              <li><span><?php echo e(strtoupper($audio->format)); ?></span></li>
            <?php endif; ?>
          </ul>

          <?php if($audio->description): ?>
            <h3><?php echo strip_tags(Str::limit(stripslashes($audio->description),350)); ?></h3>
          <?php endif; ?>
        </div>
      </div>
    </div>
                ...
                <?php $__currentLoopData = $related_audios; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $related_audio): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                  <div class="single-video audio-card-related">
                    <div class="audio-card-content">
                      <div class="audio-play-btn" onclick="toggleRelatedAudio(<?php echo e($related_audio->id); ?>)">
                        <i class="fa fa-play" id="playIcon<?php echo e($related_audio->id); ?>"></i>
                      </div>

                      <div class="audio-info">
                        <h3 class="audio-title"><?php echo e(Str::limit(stripslashes($related_audio->title),25)); ?></h3>
                        <?php if($related_audio->genre): ?>
                          <span class="audio-genre"><?php echo e($related_audio->genre); ?></span>
                        <?php endif; ?>
                        <?php if($related_audio->duration): ?>
                          <span class="audio-duration"><?php echo e($related_audio->duration); ?></span>
                        <?php endif; ?>
                      </div>

                      <div class="audio-actions">
                        <a href="<?php echo e(URL::to('audio/details/'.$related_audio->id)); ?>" class="action-btn" title="View Details">
                          <i class="fa fa-eye"></i>
                        </a>
                        <?php if(!$related_audio->license_price || $related_audio->license_price == 0): ?>
                          <a href="<?php echo e(URL::to('audio/download/'.$related_audio->id)); ?>" class="action-btn" title="Download">
                    ...
                    <!-- Hidden Audio Player -->
                    <audio id="audioPlayer<?php echo e($related_audio->id); ?>" preload="none">
                      <source src="<?php echo e(asset('storage/' . $related_audio->audio_path)); ?>" type="audio/<?php echo e($related_audio->format ?: 'mp3'); ?>">
                    </audio>
                  </div>
...
<?php echo $__env->make('site_app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\xampp\htdocs\stock_new\resources\views/pages/sports/details.blade.php ENDPATH**/ ?>