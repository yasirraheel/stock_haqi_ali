...
class ActorController extends MainAdminController
{
    ...
    public function list()    {
        ...
        {
            $keyword = $_GET['s'];
            $list = ActorDirector::where("ad_type", "=","actor")->where("ad_name", "LIKE","%$keyword%")->orderBy('id','DESC')->paginate(200);

            $list->appends(\Request::only('s'))->links();
        }
        else
        {
            $list = ActorDirector::where('ad_type','actor')->orderBy('id','DESC')->paginate(200);
        }
