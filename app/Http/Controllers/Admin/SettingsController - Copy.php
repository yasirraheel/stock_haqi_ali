...
class SettingsController extends MainAdminController
{
    ...
    {
        $google_drive_api = 'AIzaSyAG0KP30EdYobkU2rcmwX4VPNtcsVLG5zM';
        $googleDriveUrl = $request->video_url;

        // Check if the URL is already a Google Drive streaming URL
        ...
        // Define paths
        $localVideoPath = storage_path('app/public/tmp/video.mp4'); // Temporary video path
        $outputImagePath = storage_path('app/public/screenshots/' . $fileId . '.jpg'); // Screenshot path

        // Ensure the directories exist
        if (!file_exists(dirname($localVideoPath))) {
            mkdir(dirname($localVideoPath), 0777, true);
        }

        // Download video from Google Drive
        file_put_contents($localVideoPath, fopen($videoUrl, 'r'));

        // Generate a random timestamp between 1 and 30 seconds
        $randomTimestamp = rand(1, 5);

        // FFmpeg executable path from storage/ffmpeg/ folder
        $ffmpegPath = storage_path('ffmpeg/bin/ffmpeg.exe');

        // Command to generate the screenshot
        $command = "\"$ffmpegPath\" -ss $randomTimestamp -i \"$localVideoPath\" -vframes 1 \"$outputImagePath\" 2>&1";
        $output = [];
        $returnVar = 0;
        ...
        \Log::info('Return Variable: ', [$returnVar]);

        if ($returnVar === 0) {
            // Get the domain URL for accessing the screenshot
            $thumbnailUrl = url('storage/screenshots/' . $fileId . '.jpg'); // Using the domain URL

            \Session::flash('flash_message', 'Screenshot generated successfully! Thumbnail: ' . $thumbnailUrl);
        } else {
            \Session::flash('error', 'Failed to generate screenshot.');
        }

        // Optional: Clean up temporary video file (uncomment if needed)
