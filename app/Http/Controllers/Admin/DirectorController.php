...
class DirectorController extends MainAdminController
{
    ...
    public function list()    {
        ...
        {
            $keyword = $_GET['s'];
            $list = ActorDirector::where("ad_type", "=","director")->where("ad_name", "LIKE","%$keyword%")->orderBy('id','DESC')->paginate(100);

            $list->appends(\Request::only('s'))->links();
        }
        else
        {
            $list = ActorDirector::where('ad_type','director')->orderBy('id','DESC')->paginate(100);
        }
